...
public class FileWatcher implements ApplicationContextAware {
    ...
    private String watchFile = "/Users/zhengpeng/space/git/hodor";
                ...
                try {
                    ...
                    for (WatchEvent<?> watchEvent : key.pollEvents()) {
                        ...
                        if (kind == StandardWatchEventKinds.ENTRY_MODIFY) {
                            ...
                            int status = javac.run(null, null, null, "-d", this.getClass().getResource("/").getPath(), String.format("%s/%s", key.watchable().toString(), filePath.toString()));
                            if (status != 0) {
                                LOGGER.info("没有编译成功！");
                            } else {
...
//                              String classPathName = path.substring(1).replaceAll("/", "\\.") + "." + className;
